...
// ########################## PROGRESSÃO DESMATAMENTO MÊS A MÊS ###########################
...
var inputStartYear = 2020;              // Ano inicial do estudo
var inputStartMonth = 1;                // Mês inicial do estudo
var inputEndYear = 2021;                // Ano final do estudo
var inputEndMonth = 12;                 // Mês final do estudo (12 para dezembro)
var aoiCircleBuffer = 4;                
var inputCloudPercent = 50;             // Porcentagem máxima de nuvem permitida na imagem
...
// Intervalo inicial de meses e anos para composição mensal
var startYearUrl = ui.url.get('startYear', String(inputStartYear));
ui.url.set('startYear', startYearUrl);

var startMonthUrl = ui.url.get('startMonth', String(inputStartMonth));
ui.url.set('startMonth', startMonthUrl);

var endYearUrl = ui.url.get('endYear', String(inputEndYear));
ui.url.set('endYear', endYearUrl);

var endMonthUrl = ui.url.get('endMonth', String(inputEndMonth));
ui.url.set('endMonth', endMonthUrl);
...
var aboutLabel = ui.Label(
  ...
  'locations within image composites. Images are median monthly composites ' +
  'generated for a given time window. Time series ' +
...
var startYearBox = ui.Textbox({value: ui.url.get('startYear'), style: textFont});
startYearBox.style().set('stretch', 'horizontal');

var startMonthLabel = ui.Label('Mês inicial', textFont);
var startMonthBox = ui.Textbox({value: ui.url.get('startMonth'), style: textFont});
startMonthBox.style().set('stretch', 'horizontal');

var endYearLabel = ui.Label('Ano final', textFont);
var endYearBox = ui.Textbox({value: ui.url.get('endYear'), style: textFont});
endYearBox.style().set('stretch', 'horizontal');

var endMonthLabel = ui.Label('Mês final', textFont);
var endMonthBox = ui.Textbox({value: ui.url.get('endMonth'), style: textFont});
endMonthBox.style().set('stretch', 'horizontal');
...
var exportPanel = ui.Panel([
  ...
  ui.Panel([startMonthLabel, startMonthBox], ui.Panel.Layout.Flow('horizontal'), {stretch: 'horizontal'}),
  ui.Panel([endYearLabel, endYearBox], ui.Panel.Layout.Flow('horizontal'), {stretch: 'horizontal'}),
  ui.Panel([endMonthLabel, endMonthBox], ui.Panel.Layout.Flow('horizontal'), {stretch: 'horizontal'}),
...
// Valida mês informado entre 1 e 12
...
// Gera mosaico mensal com metadados para o card
function imgColPlan(monthStart, aoiBox){
  var maxCloud = parseNumber(cloudBox.getValue(), inputCloudPercent);
  var monthEnd = monthStart.advance(1, 'month');
  var s2Col = getCollection(monthStart, monthEnd, aoiBox, maxCloud);
  var tiles = s2Col.aggregate_array('MGRS_TILE').distinct().sort();
  var orbitPointText = ee.String('Tile: ')
    .cat(ee.String(tiles.join(', ')));
  var mosaic = s2Col.median().clip(aoiBox);
...
function renderGraphics(coords) {
  ...
  var currentStartMonth = parseMonth(startMonthBox.getValue(), inputStartMonth);
  var currentEndMonth = parseMonth(endMonthBox.getValue(), inputEndMonth);
  ...
  var startMonth = currentStartMonth;
  var endMonth = currentEndMonth;

  // Calcula janela mensal e gera coleção
  var startDate = ee.Date.fromYMD(startYear, startMonth, 1);
  var endDateCandidate = ee.Date.fromYMD(endYear, endMonth, 1).advance(1, 'month');
  var endDate = ee.Date(ee.Algorithms.If(endDateCandidate.millis().lt(startDate.millis()), startDate.advance(1, 'month'), endDateCandidate));
  var monthCount = endDate.difference(startDate, 'month');
  var monthOffsets = ee.List.sequence(0, ee.Number(monthCount).subtract(1));
  var col = ee.ImageCollection.fromImages(
    monthOffsets.map(function(offset) {
      var monthStart = startDate.advance(ee.Number(offset), 'month');
      return imgColPlan(monthStart, aoiBox);
    })
...
function setParams() {
  ...
  ui.url.set('startYear', startYearBox.getValue());
  ui.url.set('startMonth', startMonthBox.getValue());
  ui.url.set('endYear', endYearBox.getValue());
  ui.url.set('endMonth', endMonthBox.getValue());
}
...
startMonthBox.onChange(optionChange);
endYearBox.onChange(optionChange);
endMonthBox.onChange(optionChange);
